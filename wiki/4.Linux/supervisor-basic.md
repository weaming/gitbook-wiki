...
- 每次修改主配置文件或增改子配置文件都需要执行`supervisorctl update`使新配置生效。或者用`supervisorctl reload`重新重新启动supervisor程序。
- 输入`supervisorctl`打开supervisor的命令行控制台，输入`help`查看命令，输入`help 命令名`查看具体命令帮助
- `supervisorctl  stop program_name` 停止程序
- `supervisorctl start all`启动 所有

### 开启web端图形控制界面
取消注释并修改以下区段
```
[inet_http_server]
port=127.0.0.1:9001  ;可以改成 0.0.0.0
username=user
password=123
```
![](http://neilimg.b0.upaiyun.com/code/supervisor-web-console.png)
...
;process_name=%(program_name)s ; 这个是进程名，如果我们下面的numprocs参数为1的话，就不用管这个参数
                                 ...
                                 进程都用同一个进程名吧。weaming注：%(program_name)s_%(process_num)02d
